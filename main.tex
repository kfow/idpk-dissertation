...
Testing necessitated the restructuring of some of the files which read and write information to files. Let us observe an example in the form of the TargetDocument.java class. This class analyses a gzipped target document to create a representation which can be converted to JSON and delivered to the Web App for viewing.
...
public TargetDocument(String path){
		try {
            ...
            ...
\end{lstlisting}
This approach is inflexible as it assumes the file path will direct to a gzipped file.
A better, more general, approach is to have the constructor take a \code{Reader} interface as an argument. We can then instantiate the necessary \code{BufferedReader} needed.
\begin{lstlisting}
...
Integration aims to achieve something quite different to unit testing. This tests the behaviour and responses of the API Methods rather than any of their underlying objects. This is important in order to identify correct response codes were being sent as a result of various requests.
\section{Conclusion}
